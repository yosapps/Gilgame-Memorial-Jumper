...
func _ready() -> void:
	...
	skip_label = Label.new(); skip_label.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); skip_label.position = Vector2(-220, -45); skip_label.size = Vector2(200, 30); skip_label.text = "Esc / Enter  スキップ"; skip_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT; background.add_child(skip_label)
	audio = AudioStreamPlayer.new(); add_child(audio)
	background.hide()

func _unhandled_input(event: InputEvent) -> void:
	if playing and (event.is_action_pressed("ui_cancel") or event.is_action_pressed("ui_accept")):
		skip_requested = true
...
func play_memory(data: MemoryCrystalData) -> void:
	...
	MemoryManager.memory_sequence_started.emit(data)
	...
	for texture in data.memory_images:
		...
		if texture == null:
			push_warning("Missing image in %s" % data.crystal_id); continue
		...
		var motion := create_tween(); motion.set_parallel(true)
		motion.tween_property(image_rect, "scale", Vector2(1.04, 1.04), data.seconds_per_image).set_trans(Tween.TRANS_SINE)
		motion.tween_property(image_rect, "position", Vector2(-12, -7), data.seconds_per_image).set_trans(Tween.TRANS_SINE)
		await get_tree().create_timer(data.seconds_per_image, true).timeout
		if skip_requested: break
	...
	MemoryManager.memory_sequence_finished.emit(data)
