...
func _ready() -> void:
	...
	counter = Label.new()
	counter.position = Vector2(18, 52)
...
func _unhandled_input(event: InputEvent) -> void:
	if (
		...
		and not event.echo and event.physical_keycode == KEY_M
	):
		if panel.visible:
			close_menu()
		else:
...
func open_menu() -> void:
	for child in list.get_children():
		child.queue_free()
	var title := Label.new()
	title.text = "MEMORIES     %d / 10" % MemoryManager.get_collected_count()
	...
	for index in range(1, 11):
		...
		button.text = "%02d  %s" % [
			index,
			data.display_name if MemoryManager.is_collected(data.crystal_id) else "???",
		]
		button.disabled = not MemoryManager.is_collected(data.crystal_id)
		button.pressed.connect(_replay.bind(data))
		list.add_child(button)
	panel.show()
	GameTimeManager.pause_timer(&"memory_menu")


func close_menu() -> void:
	panel.hide()
	GameTimeManager.resume_timer(&"memory_menu")
